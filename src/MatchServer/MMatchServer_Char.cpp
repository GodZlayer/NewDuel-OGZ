...
bool MMatchServer::ResponseDeleteChar(const MUID& uidPlayer, const int nCharIndex, const char* szCharName)
{
	...
	if ((nCharIndex < 0) || (nCharIndex >= MAX_CHAR_COUNT)) return false;
						 ...
						 const unsigned int nCostume, const unsigned int nRace, const unsigned int nBodyType)
	ResponseCreateChar(uidPlayer, nCharIndex, szCharName, MMatchSex(nSex), nHair, nFace, nCostume, nRace, nBodyType);
						const unsigned int nCostume, const unsigned int nRace, const unsigned int nBodyType)

											szCharName, nCharIndex, nSex, nHair, nFace, nCostume, nRace, nBodyType);
}
